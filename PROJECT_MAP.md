...
> Обновлено: 2026-09-12 · актуально для v1.9.2 «Клинок Живёт»
...
| BobliksSettings.uc.js | 1.14.0 | Меню «B»: темы (+авто-тема день/ночь), фоны, DNS, ПЕРФ-замеры, бэкап-кнопка, система; F1/Alt+B. `applyThemeSheet()` — USER_SHEET для живого переключения тем |
...
| BladeClock.uc.js | 2.3.0 | Часы+погода+прогноз в тулбаре (ipwho.is + open-meteo), публикует в шину clock:weather |
...
| 1.9.2 | Клинок Живёт | 13 микро-вау (CSS — Gemini/Antigravity, селекторы сверены: [dragtarget] tabs.css:209, [privatebrowsingmode]; JS-хуки контрактов — GLM): искры закрытия (.blade-ghost), разрез открытия ([fadein]), перековка тем (transition), вспышка окна (.blade-flash), блик кнопки B, разрядка капсулы (.blade-loaded), воспламенение короны, шиммер окна, звёздная пыль приватки, глиф погоды в часах, карточка драга, пульс ввода палитры (.blp-typing), закатная волна. Не опубликован — на визуальной проверке |
