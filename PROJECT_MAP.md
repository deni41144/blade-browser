...
| 1.7.7 | Родной Дом | Внешние ссылки открываются в профиле Blade: команды реестра (StartMenuInternet + BladeURL/BladeHTML) теперь с -profile; set-blade-default.ps1 принимает EnginePath и корнем, и папкой движка. Друзьям после апдейта — один клик «Сделать браузером по умолчанию». Не опубликован — на проверке |
