...
> Обновлено: 2026-09-12 · актуально для v1.9.3 «Дом без Эха»
...
| BladeNewtab.uc.js | 1.3.0 | Hero на новой вкладке: приветствие по времени, дата, статус-строка (BLADE OS // ONLINE · погода), тикер хроники (chrome-оверлей; данные Clock через шину BladeCore). Часы вырезаны в 1.3.0 — время/погода дублировали навбар-часы, тик 1с→30с |
| BladeClock.uc.js | 2.5.2 | Часы+погода+прогноз в тулбаре (ipwho.is + open-meteo), публикует в шину clock:weather |
...
| BobliksChromeStyle 1.3.0 / AboutStyle 1.0.1 / BladeSounds 1.2.0 | — | Стили хрома/about (incl. порт SIGNATURE), звуки |
...
| 1.9.3 | Дом без Эха | Дедупликация главной: большие часы Hero вырезаны (BladeNewtab 1.3.0 — время/погода уже в навбар-часах; остались приветствие+дата+статус, тик 1с→30с). Движковый виджет clocks был выключен префами и в user.js (browser.newtabpage.activity-stream.widgets.clocks.enabled=false — пин для друзей). Из патч-конвейера навсегда исключены диагностические BladeDiag/BladeDiag2.uc.js (в 1.9.2 они случайно попали в непубликовавшийся архив). Ожидает «публикуй» |
