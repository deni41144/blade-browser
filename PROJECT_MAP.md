...
> Обновлено: 2026-09-12 · актуально для v1.9.5 «Голос Клинка»
...
| BladeNewtab.uc.js | 1.4.0 | Hero поверх ВСЕХ страниц: циферблат+приветствие+дата+статус (chrome-оверлей; погода из Clock через шину BladeCore). На about:newtab/home часы скрываются (`.blade-no-clock` — дубль навбар-часов), хроника обновлений — только на главной. Тёмная подложка-градиент для светлых сайтов, гвард фуллскрина (window.fullScreen/fullscreenElement)
...
12. **Вето владельца на фичи «для галочки»** (2026-09-12, слова Дени): «есть
   моменты — всякие замудрённые хоткеи, режим чтения и т.д. — 90% людей не
   юзают; тема в определённый день — сильная нагрузка на ядро, а пользы
   ноль». НЕ предлагать и НЕ делать: лабиринты хоткеев, режимы
