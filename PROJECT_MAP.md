...
> Обновлено: 2026-09-12 · актуально для v1.7.6 «Живое Сердце»
...
| BobliksSettings.uc.js | 1.10.1 | Меню «B»: темы (+авто-тема день/ночь), фоны, DNS, ПЕРФ-замеры, бэкап-кнопка, система; F1/Alt+B. `applyThemeSheet()` — USER_SHEET для живого переключения тем |
...
| 1.7.6 | Живое Сердце | Кнопка B восстановлена: FF155 строил голую кнопку без .toolbarbutton-icon (весь облик/пульсации тем висели на нём) — виджет переведён на type:custom+onBuild с ребёнком-иконкой. Не опубликован — на визуальной проверке (что скажет про тулбокс-эффекты) |
