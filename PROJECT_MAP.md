...
> Обновлено: 2026-09-12 · актуально для v1.7.7 «Родной Дом»
...
| BobliksSettings.uc.js | 1.10.2 | Меню «B»: темы (+авто-тема день/ночь), фоны, DNS, ПЕРФ-замеры, бэкап-кнопка, система; F1/Alt+B. `applyThemeSheet()` — USER_SHEET для живого переключения тем |
...
| BobliksChromeStyle 1.3.0 / AboutStyle 1.0.1 / BladeSounds 1.0.1 | — | Стили хрома/about (incl. порт SIGNATURE), звуки |
...
| 1.7.7 | Родной Дом | Внешние ссылки открываются в профиле Blade: команды реестра (StartMenuInternet + BladeURL/BladeHTML) теперь с -profile; set-blade-default.ps1 принимает EnginePath и корнем, и папкой движка. Друзьям после апдейта — один клик «Сделать браузером по умолчанию». Не опубликован | — на визуальной проверке (что скажет про тулбокс-эффекты) |
...
11. **-Notes без ASCII-кавычек:** PS 5.1 не экранирует  при передаче аргументов нативному процессу — текст нотсов с  рвёт сборку (PositionalParameterNotFound). Писать «ёлочки». (Урок 1.7.7)
