...
> Обновлено: 2026-09-15 (ночь) · актуально для v2.0.4 «Кровавый Циферблат» — **ОПУБЛИКОВАН** (GitHub Releases, Patch 10.3МБ, Latest; апдейтер друзей подхватит за сутки / «Проверить сейчас» в меню B)
...
│           ├── JS\              17 uc.js-скриптов (загрузка через utils\ fx-autoconfig)
...
| BladeNewtab.uc.js | 1.7.1 | Hero ТОЛЬКО на новой вкладке: per-theme шрифты и эффекты 9 тем (v1.7.1: кастомный ttf-шрифт BladeBlood с впаянными каплями крови прямо в контуры глифов цифр; устранены перекрытия шрифтов спанами — все 9 тем имеют свои шрифты без сбоев), большие часы 88px+секунды, приветствие, дата, статус-строка, тикер хроники. На остальных страницах время — навбар-часы (BladeClock 2.6.0)
